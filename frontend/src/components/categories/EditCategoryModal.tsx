...
export const EditCategoryModal: React.FC<EditCategoryModalProps> = ({ isOpen, onClose, category }) => {
  ...
  useEffect(() => {
    if (category) {
      setFormData({
        name: category.name,
        type: category.type === 'both' ? 'expense' : category.type,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const categoryData: Partial<CategoryFormData> = {
      ...
      icon: category.icon // Keep existing icon
    };

    updateCategoryMutation.mutate({ id: category._id, data: categoryData });
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-md">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-300">Current appearance</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Color and icon will be preserved</p>
              </div>
            </div>
          </div>
